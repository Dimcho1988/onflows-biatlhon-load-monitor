...
with st.sidebar:
    ...
    lthr_default = int(df_a["hr_bpm"].quantile(0.95))
    lthr = st.slider("LTHR (праг HR) [bpm]", min_value=130, max_value=200, value=min(172, max(130, lthr_default)))
    st.caption("Зони: Z1<75% LTHR, Z2 75–85, Z3 85–92, Z4 92–100, Z5>100%")

    st.subheader("Тегла за натоварване по зона")
    st.caption("По-реалистично за ACWR демо: по-високо наказание за Z4–Z5.")
...
sport_sel = st.multiselect("Филтър средство", sorted(df_a["sport"].unique().tolist()), default=sorted(df_a["sport"].unique().tolist()))
...
chart = alt.Chart(daily_tot).transform_fold(
...
st.dataframe(daily_tot.tail(21), use_container_width=True, hide_index=True)
